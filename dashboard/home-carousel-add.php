<?php
	...
	$pageName = "Home Carousel";
	$parentPageURL = 'welcome.php';
	$pageURL = 'home-carousel-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addHomeCarousel($_POST,$_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueHomeCarouselById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg','image/png');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateHomeCarousel($_POST,$_FILES);
							...
							<div class="row">
								...
								<div class="col-sm-4">
									...
									<span class="help-text">
										...
										Images must be exactly <strong>48x48</strong> pixels.
									</span>
									<br>
									...
									?>
										<img src="../img/<?php echo $file_name.'_crop.'.$ext ?>" width="100"  />
									<?php
									} ?>
								</div> 	 
								<div class="col-sm-4">
