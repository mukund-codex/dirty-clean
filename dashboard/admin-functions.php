<?php
	...
	class AdminFunctions extends Database {
		...
		function updateWelcomeContent($data){
			...
			$query = "update ".PREFIX."gallery_content set description='".$description."',video_link='".$link."' where id='".$id."'";
			$this->query($query);
		}
			...
			if(sizeof($data['category'])>0) {
				...
				foreach($data['category'] as $key=>$value) {
					if($data['category'][$key]!=''){
						...
						$this->query("insert into ".PREFIX."faqs (category, question, answer) values ('".$category."','".$question."','".$answer."')");
						$j++;
					}
				}
			}
		...
		function getFAQs(){
			return $this->query("select * from ".PREFIX."faqs");
		}
		...
		function addSubscription($data){
			...
			$query = "insert into ".PREFIX."subscription_master(category, package_name, validity_period, package_price, permalink, active) values ('".$category."','".$package_name."', '".$validity_period."', '".$package_price."', '".$permalink."', '".$active."')";
			return $this->query($query);
		}
		...
		function updateSubscription($data) {
			...
			$query = "update ".PREFIX."subscription_master set category = '".$category."', package_name='".$package_name."', validity_period='".$validity_period."', package_price='".$package_price."', permalink='".$permalink."', active='".$active."' where id='".$id."'";
			return $this->query($query);
		}
